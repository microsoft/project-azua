...
for i in range(5):
	...
	# Save non-std data
	savedir = "miss_protein_cells_seed_%i" % (i+1)
	os.system("mkdir %s" % savedir)
	print(savedir)
	save_data(savedir, adj_matrix, X_miss, indices_train, indices_test)

	# Save std data
	savedir = "miss_protein_cells_seed_%i_std" % (i+1)
	os.system("mkdir %s" % savedir)
	print(savedir)
