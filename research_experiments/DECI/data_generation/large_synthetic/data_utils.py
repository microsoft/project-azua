...
from azua.azua.models.deci.diagonal_flows import PiecewiseRationalQuadraticTransform
import torch
...
def get_name(gt, N, E, sem_type, noise_type, seed):
    name_base = "%s_%i_%i_%s_sem_%s_noise_%d_seed" % (gt, N, E, sem_type, str(noise_type), seed)
    return name_base
...
def gen_dataset(
    ...
    if discrete_dims_list is not None:
        discrete_idxs = np.array(discrete_dims_list) != None
        std[discrete_idxs] = 1
