...
def two_node_lin(n_samples_train, n_samples_per_test, datadir, noise_dist, name):
    ...
    def lin_model():
        ...
        x1 = numpyro.deterministic("x1", 0.5 * x0 + (np.sqrt(3) / 2) * x1_noise)
...
def collider_lin(n_samples_train, n_samples_per_test, datadir, noise_dist):
    ...
    def collider_lin_model():
        ...
        x1_noise = numpyro.sample("x1_noise", noise_dist)
        x1 = numpyro.deterministic("x1", np.sqrt(1 / 3) * x0 + np.sqrt(1 / 3) * x2 + np.sqrt(1 / 3) * x1_noise)
...
def chain_lin(n_samples_train, n_samples_per_test, datadir, noise_dist):
    ...
    def chain_lin_model():
        ...
        x2 = numpyro.deterministic("x2", np.sqrt(2 / 3) * x1 + np.sqrt(1 / 3) * x2_noise)
...
def two_node_nonlinear_gauss(n_samples_train, n_samples_per_test, datadir):
    ...
    def nonlingauss_model():
        ...
        x1 = numpyro.deterministic("x1", np.sqrt(6) * np.exp(-(x0 ** 2)) + magic_number * x1_noise)
...
def nonlin_simpson(n_samples, datadir):
    ...
    def non_lin_simpson_model():
        ...
        x3 = numpyro.sample("x3", dist.Laplace(5 * jnp.tanh((x2 - 4) / 5) + 3, 0.1))
...
def symprod_simpson(n_samples, datadir):
    ...
    def product_simpson_model():
        ...
        x2 = numpyro.sample("x2", dist.Laplace(0.5 * x0 * x1, 0.5))

        x3 = numpyro.sample("x3", dist.Normal(jnp.tanh(1.5 * x0), 0.3))
...
def large_backdoor(n_samples, datadir, binary_treatment=False):
    ...
    def large_backdoor_model():
        ...
        x8 = numpyro.sample("x8", dist.Laplace(-jnp.exp((-x6 * 1.3 + x7) / 3 + 1) / 3 + 2, 0.5))
...
def weak_arrows(n_samples, datadir, binary_treatment=False):
    ...
    def weak_arrow_model():
        ...
        x8 = numpyro.sample("x8", dist.Laplace(x8_mean - 2, 0.5,))
...
def cat_chain(num_samples_train, num_samples_per_test, datadir):
    ...
    def cat_chain_model():
        ...
        x2 = numpyro.sample("x2", dist.Categorical(probs=x2_probs))
...
def cat_to_cts(num_samples_train, num_samples_per_test, datadir):
    ...
    def cat_to_cts_model():
        ...
        x1_noise = numpyro.sample("x1_noise", dist.Normal(0, 1))
        x1 = numpyro.deterministic("x1", (one_hot(x0, 3) * cond_means).sum(-1) + 1.6 * (nn.softplus(x1_noise) - 1))
...
def mixed_simpson(num_samples_train, num_samples_per_test, datadir):
    ...
    def mixed_simpson_model():
        ...
        x3 = numpyro.deterministic("x3", (10 / 3) * jnp.tanh(x1 / 3) + 0.1 * x3_noise - 0.1)
...
def mixed_confounding(num_samples_train, num_samples_per_test, datadir):
    ...
    def mixed_confounding_model():
        ...
        x9 = numpyro.deterministic("x9", x0 + 1.9 * nn.softplus(x9_noise) - 2.4)

        x10_noise = numpyro.sample("x10_noise", dist.Normal(0, 1))
        x10 = numpyro.deterministic("x10", 0.8 * x1 ** 2 + nn.softplus(x10_noise) - 1.6)

        x11_noise = numpyro.sample("x11_noise", dist.Normal(0, 1))
        x11 = numpyro.deterministic("x11", x0 - 1.5 * nn.softplus(x1) + nn.softplus(x11_noise) - 0.4)
...
def lin_gauss(n_samples, datadir):
    ...
    def linGauss_model():
        ...
        x4 = numpyro.sample("x4", dist.Normal(0.2 * x1 + 0.2 * x3 + 0.3 * x7, 0.3))
