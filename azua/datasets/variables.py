...
class Variables:
    ...
    def __init__(
        ...
        for idx, var in enumerate(self._variables + self._auxiliary_variables):
            self.var_idxs_by_type[var.type].append(idx)
        ...
        assert len(self.used_cols) == len(self._variables) + len(self._auxiliary_variables)
    ...
    @overload
    def __getitem__(self, idx: slice) -> List[Variable]:
        ...
    ...
    def __len__(self) -> int:
        return len(self._variables) + len(self._auxiliary_variables)
    ...
    def create_from_data_and_dict(
        ...
        variables_dict = cls._infer_from_data(data, mask, variables_dict, True)
    ...
    def _infer_from_data(
        ...
        infer_aux_variables: Optional[bool] = False,
    ) -> Dict[str, Any]:
        ...
    ...
    @staticmethod
    def _infer_from_data(data, mask, variables_dict=None, infer_aux_variables=False) -> Dict[str, List[Any]]:
        """
    ...
    def _all_variables(self):
        return self._variables + self._auxiliary_variables
    ...
    def has_auxiliary(self) -> bool:
        ...
        """
        return len(self._auxiliary_variables) > 0
    ...
    def non_text_idxs(self) -> List[bool]:
        ...
        return ~np.in1d(range(len(self)), unproc_cols_by_type["text"])
    ...
    def processed_cols_by_type(self) -> Dict[str, List[List[int]]]:
        ...
        for var, cols in zip(self._all_variables, self.processed_cols):
            grouped_vars[var.type].append(cols)
        return grouped_vars
    ...
    def subset(self, idxs: List[int], auxiliary_idxs: List[int] = []) -> Variables:
        """
        ...
        auxiliary_variables_list = [self._auxiliary_variables[idx] for idx in auxiliary_idxs]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self._auxiliary_variables is None:
            ...
            auxiliary_vars_list = [var.to_json() for var in self._auxiliary_variables]
    ...
    def get_observable_variable_idxs(self, data_mask_row: np.ndarray, obs_mask_row: np.ndarray) -> List[int]:
        ...
        if data_mask_row.ndim != 1:
            raise ValueError(
                "Test mask should be 1D, had %d dims and shape %s." % (data_mask_row.ndim, data_mask_row.shape)
            )
        if obs_mask_row.ndim != 1:
            raise ValueError(
                "Observation mask should be 1D, had %d dims and shape %s." % (obs_mask_row.ndim, obs_mask_row.shape)
            )
        if len(obs_mask_row) != len(data_mask_row) or len(data_mask_row) != len(self._variables):
            ...
            raise ValueError(
                "Lengths of obs_mask_row (%d), data_mask_row (%d), "
                "and variables list (%d) should all be the same."
                % (len(obs_mask_row), len(data_mask_row), len(self._variables))
            )
    ...
    @overload
    def get_var_cols_from_data(self, var_idx: int, data: np.ndarray) -> np.ndarray:
        ...

    @overload
    def get_var_cols_from_data(self, var_idx: int, data: torch.Tensor) -> torch.Tensor:
        ...
...
class Variable:
    ...
    def __init__(
        ...
        if self.type == "categorical" and self.processed_dim == 2:
            logging.info("Changing variable %s from categorical to binary." % self.name)
            self.type = "binary"
    ...
    def _try_assign(var: Any, name: str, var_type: Any) -> Any:
        ...
        if not isinstance(var, var_type):
            try:
                ...
                raise ValueError(
                    "%s must be of type %s but was type %s and could not be converted." % (name, var_type, type(var))
                )
        return var
    ...
    def processed_dim(self) -> int:
        ...
        if self.type == "continuous" or self.type == "binary":
        ...
        if not float.is_integer(float(processed_dim)):
            raise ValueError("Processed dim should be an integer, but was %f." % processed_dim)
