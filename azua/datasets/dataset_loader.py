...
class DatasetLoader(ABC):
    ...
    def negative_sample(data, data_mask, levels: Dict[int, int]):
        ...
        for row_idx in tqdm(range(num_rows), desc="Negative sampling"):
            ...
            observed_col_idxs = set([i for i in data_mask_row.nonzero()[1]])
    ...
    def _apply_negative_sampling(
        ...
        test_mask: csr_matrix,
    ) -> Tuple[csr_matrix, csr_matrix, Optional[csr_matrix], Optional[csr_matrix], csr_matrix, csr_matrix]:
        ...
    ...
    def _generate_data_split(
        ...
        """
        if isinstance(random_state, tuple) or isinstance(random_state, list):
