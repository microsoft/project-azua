...
class DoWhy(Model, IModelForInterventions):
    ...
    def __init__(
        ...
        causal_discovery_model_type: Optional[str] = "deci",
        causal_discovery_model_save_dir: Optional[str] = None,
        causal_discovery_model_config_file: Optional[str] = None,
        ...
        Args:
            ...
            causal_discovery_model_type: model type from which adjacency matrices will be extracted
            causal_discovery_model_save_dir: save directory for model from which adjacency matrices will be extracted
            causal_discovery_model_config_file: config file for model from which adjacency matrices will be extracted
        ...
        self.labels = self.variables.query_group_names
        self.categorical_columns = [
            group
            for group, idxs in zip(self.variables.query_group_names, self.variables.query_group_idxs)
            if self.variables[idxs[0]].type in ["categorical", "binary"]
        ]

        # get Causal Dag from pre-trained discovery model
        if causal_discovery_model_save_dir is not None:
            assert isinstance(causal_discovery_model_type, str)
            self.load_graph_from_discovery_save(
                causal_discovery_model_save_dir, causal_discovery_model_type, causal_discovery_model_config_file
            )
        else:
            ...
            if adj_matrix_samples is not None:
                self.graph_samples = [
                    DoWhy.str_to_dot(self.graph_from_adjacency(mat).source) for mat in adj_matrix_samples
                ]
    ...
    def _normalise_inputs(
        ...
        lowers = np.array([var.lower for var in self.variables if var.type != "text"])
        uppers = np.array([var.upper for var in self.variables if var.type != "text"])
    ...
    def _dowhy_cate(
        ...
        dowhy_model = CausalModel(
            data=train_df, treatment=self.labels[intervention_idxs[0]], outcome=self.labels[target_idx], graph=graph,
        )
    ...
    def load_graph_from_discovery_model(self, discovery_model, samples=5):
        ...
        assert self.adj_matrix.shape[0] == self.variables.num_query_groups
    ...
    def load_graph_from_discovery_save(
        self, model_save_path, model_type: str = "deci", model_config_file: Optional[str] = None, samples: int = 5
    ):
        """
        Load graphs from a pre-trained causal discovery model in a specified savefile
        """
        ...
        self.load_graph_from_discovery_model(discovery_model, samples=samples)
