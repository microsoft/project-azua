...
class DiagonalFLowBase(nn.Module):
    def __init__(self, input_dim: int, device: torch.device, num_bins: int = 8, flow_steps: int = 1) -> None:
        ...
        self.base_dist = td.Normal(
            loc=torch.zeros(self.input_dim, device=self._device),
            scale=torch.ones(self.input_dim, device=self._device),
            validate_args=None,
        )
...
class CategoricalLikelihood(nn.Module):
    def __init__(self, input_dim: int, device: torch.device):
        ...
        self.input_dim = input_dim
        self._device = device
        self.base_logits = nn.Parameter(torch.zeros(self.input_dim, device=self._device), requires_grad=True)
    ...
    def sample(self, n_samples: int):
        """
            ...
            Returns:
                samples (Nsamples, input_dim)
        """
        dist = td.Gumbel(torch.tensor(0.0, device=self._device), torch.tensor(1.0, device=self._device))
...
class BinaryLikelihood(nn.Module):
    ...
    def posterior(self, x: torch.Tensor, logit_deltas: torch.Tensor):
        ...
        log_prob_non_sampled = -self.softplus(logits * x - logits * (1 - x))
        positive_sample = self.softplus(top_sample - dist.sample(x.shape) - log_prob_non_sampled)
