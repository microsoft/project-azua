...
class TransformerSetEncoder(SetEncoderBaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._input_embedding_dim = self._feature_embedder.output_dim
        ...
        self._empty_set_embedding = torch.zeros((self._set_embedding_dim,), device=self._device, requires_grad=False)
        self.to(device)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if self._include_all_vars:
            ...
            feature_embedded_x = self._feature_embedder(x)  # Shape (batch_size * input_dim, input_embedding_dim)
            ...
            set_embedding = torch.empty((batch_size, self._set_embedding_dim), device=self._device)
