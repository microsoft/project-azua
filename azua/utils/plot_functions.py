...
def violin_plot_imputations(
    ...
    for var_idx, var in enumerate(variables):
        plot_this_var = (imputations_stats[var_idx]["type"] == "continuous") and (var.type == "continuous")
        if plot_this_var:
    ...
    if save_figs:
        if plot_name is None:
            plot_name = "violin_plot_user_id%d" % user_id
    ...
    user_stats = {"variable_names": continuous_variables_names}
    for i in range(5):
...
def bar_plot_from_per_var(
    ...
    Args:
        ...
        colours (list): List of matplotlib colours to use for the bars of the plot. At least as many colours must be passed as
    ...
    if len(lbls) > 2:
        raise ValueError(
            "Cannot plot more than 2 different axis. {} different labels found in labels.".format(
    ...
    colors = {plot: color for plot, color in zip(plots, colours[: len(plots)])}

    for label in lbls:
        for plot in plots:
            if labels[plot] == label:
                bar[plot] = ax[label].bar(
                    x_ticks + x_pos[plot],
                    mean[plot],
                    width=bar_width,
                    align="edge",
    ...
    plt.title(title)
    plt.legend([bar[plot] for plot in plots], [plot for plot in plots])
...
def plot_rewards_scatter(
    ...
    plt.clf()
    fig, axes = plt.subplots(1, max_steps, figsize=(12, 5))
...
def plot_rewards_hist(rewards_list, save_dir):
    ...
    fig, axs = plt.subplots(row_count, col_count, figsize=(25, 15))
...
def plot_difficulty_curves(strategy, observations, variables, difficulty, save_dir, steps=None):
    ...
    plt.xticks(xs)
    plt.xlabel("Steps")
    plt.ylabel(str(strategy) + "_" + "Question difficulty")
...
def plot_target_curves(imputed_values_per_strategy, save_dir):
    ...
    plt.clf()
    plt.figure(figsize=(6.4, 4.8))
    save_path = os.path.join(save_dir, "{}.png".format("Y"))
...
def plot_mean_target_curves(imputed_values_per_strategy, max_steps, save_dir):
    ...
    plt.clf()
    plt.figure(figsize=(6.4, 4.8))
    save_path = os.path.join(save_dir, "{}.png".format("mean_Y"))
...
def plot_time_results_hist(imputed_values_per_strategy, max_steps, save_dir):
    logger = logging.getLogger()

    plt.clf()
    plt.figure(figsize=(6.4, 4.8))
    save_path = os.path.join(save_dir, "{}.png".format("results_time_hist"))
    ...
    for strategy, imputed_values in imputed_values_per_strategy.items():
        ...
        user_count, step_count, feature_count = imputed_values.shape
