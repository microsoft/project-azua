...
def save_metrics_and_confusion_plot(metrics: Dict, save_file: str):
    ...
    logger.info("Metrics: " + format_dict_for_console(metrics))
...
def compute_metrics(imputed_values, ground_truth, target_mask, variables: Variables) -> Dict[str, Any]:
    ...
    if any(x.type == "binary" for x in variables):
    ...
    if any((x.type == "categorical" or x.type == "binary") for x in variables):
...
def get_metric(
    ...
    unique_variables_types = list(set([variables[i].type for i in idxs]))
    ...
    if variables_type == "continuous":
        ...
        raise ValueError(
            "Incorrect variable type. Expected one of continuous, binary or categorical. Was %s." % variables_type
        )
...
def get_rmse(imputed_values, ground_truth, target_mask, variables: Variables, normalise: bool, batch_size=10000):
    ...
    for start_idx in range(0, num_rows, batch_size):
        ...
        if normalise:
            ...
            lowers = np.array([var.lower for var in variables if var.type != "text"])
            uppers = np.array([var.upper for var in variables if var.type != "text"])
...
def compute_save_additional_metrics(
    ...
    info_gains = objective.get_information_gain(data, np.ones_like(data, dtype=bool), np.zeros_like(data, dtype=bool))
    quality = list(np.mean(info_gains, axis=0))

    # Get question difficulty for each question based on predicted percentage of incorrect answers across students
    difficulty = np.mean(1 - imputations, axis=0)

    save_difficulty = os.path.join(path, "difficulty.csv")
    save_quality = os.path.join(path, "quality.csv")

    with open(save_difficulty, "w") as save_file:
        writer = csv.writer(save_file, delimiter=",")
        for index, value in enumerate(difficulty):
            writer.writerow([index, value])

    with open(save_quality, "w") as save_file:
        writer = csv.writer(save_file, delimiter=",")
...
def get_area_under_ROC_PR(
    ...
    if binary_target_mask.sum() > 0:
        ...
        fpr, tpr, thr = sk_metrics.roc_curve(y, pred)
        AUROC = sk_metrics.auc(fpr, tpr)
        precision, recall, thr = sk_metrics.precision_recall_curve(y, pred)
