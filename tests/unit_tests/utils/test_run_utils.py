...
from azua.utils.run_utils import find_all_model_dirs, find_local_model_dir
...
def test_ignore_sub_models(tmpdir):
    with open(os.path.join(tmpdir, "model.pt"), "w"):
        pass
    sub_model_dir = os.path.join(tmpdir, "junk")
    os.mkdir(sub_model_dir)
    with open(os.path.join(sub_model_dir, "model.pt"), "w"):
        # Create empty file
        pass
    d, mid = find_local_model_dir(tmpdir)
    assert d == tmpdir
...
def test_find_two_model_dirs(tmpdir):
    ...
    with open(os.path.join(second_sub_model_dir, "model.pt"), "w"):
        pass
